...
friends["aurora@gmail.com"] = "aury"
friends["mike@gmail.com"] = "Micheal"

friends_pre_populated = {
    "bob@gmail.com" : "bob",
    "aurora@gmail.com" : "aury",
    "mike@gmail.com" : "Micheal",
}

print(friends)
print(friends_pre_populated)
...
for year, movie in films.items():
    print(f"year: {year} title: {movie} position in list: {count}")
    count += 1
...
for name, info in shelter_pets.items():
    if not info["Adopted"]:
        print(name)
...
for year, albums in dylan_albums.items():
    print(f"{year} : {albums}")
...
found_album = 1963 in dylan_albums
print(found_album)
...
for item in person:
    if person[item] not in result.values():
...
def find_max_score(scores):
    ...
    for student in scores.keys():
        if scores[student] >= max[1]:
            max = [student, scores[student]]
